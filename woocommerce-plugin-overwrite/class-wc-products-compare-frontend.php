...
class WC_Products_Compare_Frontend {
	...
	public function load_scripts() {
		...
		$max_products = apply_filters( 'woocommerce_products_compare_max_products', 3 );
	...
	public static function get_product_meta_headers( $products = array() ) {
		...
		foreach ( $products as $product ) {
			...
			$description = version_compare( WC_VERSION, '3.0.0', '<' ) ? $product->post->post_content : $product->get_short_description();
